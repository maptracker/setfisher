...
Optional Arguments:
 ...
 -eclevels This series maps evidence codes to an integer 'score' that
           will be used to represent each locus->GO assignment in the
           MatrixMarket file. 'Worse' evidence codes should be listed
           first. The default ranking is generally reasonable, but
           also fairly arbitrary on a fine scale.

           If this parameter is not provided a default value is
           provided by the GoMetadata.pm module.

-rellevels Factor levels for GeneOntology relationship types.
...
sub ontology_go {
    ...
    while (<$fh>) {
        ...
        map { s/^\-$// } @row; # Turn '-' cells to empty string
        ## Just record GOID/EC pairs for the gene:
        push @{$goLinks{$row[ $lInd ]}}, [ $row[$gInd], $row[ $eInd ] ];
    }
    close $fh;
    return &make_go_matrix( \%goLinks, $geneMeta, $trg, \%fbits, $fmeta,
...
sub species_meta_hash {
    ...
    my $tid = $td->{taxid};
    my $sci = &species_scientific_name( $td );
